...
class TestWebSearchFallback:
    ...
    async def test_missing_tool_call_falls_back_to_compound_beta(self):
        """
        When tool_call is None and no keyword matches, compound-beta-mini is used.
        Uses a query without news/weather/sports keywords so keyword router returns None.
        Note: "who won IPL 2025" now correctly routes to news MCP — that is right.
        """
        from app.agent_engine import _dispatch_tool
        ...
        with patch("app.agent_engine._compound_beta_search", new_callable=AsyncMock) as mock:
            ...
            result = await _dispatch_tool(
                None, "explain quantum entanglement simply", "chat-1", facts={}
            )

        mock.assert_called_once_with("explain quantum entanglement simply", "chat-1")
        assert result == "Compound-beta result"

    @pytest.mark.asyncio
    async def test_ipl_routes_to_news_not_web_search(self):
        """IPL keyword correctly intercepted by router → MCP news (not web_search)."""
        from app.agent_engine import _keyword_tool_from_query
        from app.tools import NewsTool
        tc = _keyword_tool_from_query("who won IPL 2025", {})
...
class TestToolDispatchRetryAfterToolFails:
    @pytest.mark.asyncio
    async def test_failed_tool_returns_empty_string_agent_exits_early(self):
        """
        When a tool returns empty string (e.g. MCP server is down), the agent
        exits early via search_empty_exit rather than looping.
        This prevents the 3-iteration spin seen when GNews returns 0 articles.
        """
        from app.agent_engine import run_agent
        ...
        assert orch_calls[0] >= 1  # at least one orchestrate; exits early on empty
