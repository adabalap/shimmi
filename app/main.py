...
Changes vs v3.2.0:
  FIX-1  "Is typing" now shown immediately at enqueue (was only shown after worker
         picked up message — gap of 1–5s during busy periods).
         CHAT_TYPING_EVENTS dict holds per-chat stop-events created at webhook
         enqueue time; process_message pops and reuses them.
  IMPR-1 Ambient memory (bot_inferred facts) now fed to orchestrator.
         source_filter changed from "user_stated" to "all" so facts extracted
         from non-prefixed group messages reach the LLM prompt.
  IMPR-2 consolidate_user_facts() now guarded by facts >= 5 threshold to avoid
         burning 8B model quota on sparse user profiles.
"""
    ...
    async with Trace(event_id=event_id, chat_id=chat_id, sender_id=sender_id) as trace:
        ...
        try:
            ...
            with trace.step("facts_load"):
                # IMPROVEMENT: Load both user_stated AND bot_inferred facts so the
                # orchestrator benefits from ambient memory extracted from group/no-prefix
                # messages.  bot_inferred facts are labelled in the prompt so the LLM
                # weights them accordingly, but they ARE fed in (previously they were
                # silently discarded and only ever used for future consolidation).
                facts = (
                    await database.sqlite_store.get_all_facts(
                        sender_key, source_filter="all"
                    )
                ...
                if facts:
                    ...
                    # IMPROVEMENT: cooldown is enforced inside consolidate_user_facts
                    # (1h default). Only schedule when user has ≥5 facts to keep
                    # the cheap 8B model from wasting quota on tiny profiles.
                    if len(facts) >= 5:
                        asyncio.create_task(
