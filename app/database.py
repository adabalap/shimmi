...
class SQLiteMemory:
    ...
    async def get_users_with_key(self, key: str) -> list:
        """
        Return all users who have the given fact key, along with its value and
        updated_at timestamp.  Used by the scheduler to find users with stale
        conversation summaries.

        Returns: list of (whatsapp_id, fact_value, updated_at) tuples.
        """
        async with self._lock:
            def _do():
                with sqlite3.connect(self.path) as conn:
                    cur = conn.execute(
                        "SELECT whatsapp_id, fact_value, updated_at FROM user_memory "
                        "WHERE fact_key=? ORDER BY updated_at ASC",
                        (key,),
                    )
                    return cur.fetchall()
            return await asyncio.to_thread(_do)
