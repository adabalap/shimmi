...
class ChromaAmbient:
    def __init__(self, persist_dir, collection_name: str, embed_model: str):
        ...
        logger.info("📚 chroma.ready dir=%s collection=%s", persist_dir, collection_name)
    ...
    async def search(self, *, chat_id: str, query: str, k: int) -> List[ContextSnippet]:
        ...
        return [ContextSnippet(id=i, text=d, metadata=m or {}, distance=di) for i, d, m, di in zip(ids, docs, metas, dists)]
    ...
    async def recent_window(self, *, chat_id: str, k: int) -> List[ContextSnippet]:
        ...
        items = [ContextSnippet(id=i, text=d, metadata=m or {}, distance=None) for i, d, m in zip(res.get("ids", []), res.get("documents", []), res.get("metadatas", []))]
