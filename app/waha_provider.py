"""
waha_provider.py — Shimmi v3.7.0
...
async def start_typing(chat_id: str) -> None:
    """Send startTyping to WAHA using the raw chat_id exactly as received."""
    if not HTTPX_WAHA:
        return
    try:
        ...
        logger.info("⌨️  typing.start  chat=%s  status=%d", chat_id, resp.status_code)
    except Exception as exc:
        logger.warning("typing.start_fail  chat=%s  err=%s", chat_id, str(exc)[:120])


async def stop_typing(chat_id: str) -> None:
    """Send stopTyping to WAHA using the raw chat_id exactly as received."""
    if not HTTPX_WAHA:
        return
    try:
        resp = await HTTPX_WAHA.post(
        ...
        logger.info("⌨️  typing.stop  chat=%s  status=%d", chat_id, resp.status_code)
    except Exception as exc:
